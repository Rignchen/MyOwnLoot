...
$execute store success score #temp mol.chest_looted run data modify block ~ ~ ~ Items set from entity @s item.tag.mol.Items[{Owner:$(owner)}].Items
execute if score #temp mol.chest_looted matches 0 run scoreboard players add @p[tag=mol.temp] mol.chest_looted 1
